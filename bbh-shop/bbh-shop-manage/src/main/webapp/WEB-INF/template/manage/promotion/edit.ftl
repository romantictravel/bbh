...
<title>${message("manage.promotion.edit")}</title>
...
$().ready(function() {
	...
	// 商品选择
	$productSelect.autocomplete("product_select.do", {
		dataType: "json",
		max: 20,
		width: 600,
		scrollHeight: 300,
		...
		[@compress single_line = true]
			...
			'<tr class="productTr">
				...
				<\/td>
				<td>
					<a href="${base}' + item.path + '" target="_blank">[${message("manage.common.view")}]<\/a>
					<a href="javascript:;" class="deleteProduct">[${message("manage.common.delete")}]<\/a>
				<\/td>
			<\/tr>';
		[/@compress]
		$productTitle.show();
	...
	// 赠品选择
	$giftSelect.autocomplete("gift_select.do", {
		dataType: "json",
		max: 20,
		width: 600,
		scrollHeight: 300,
		...
		[@compress single_line = true]
			...
			'<tr class="giftTr">
				...
				<td>
					<a href="${base}' + item.path + '" target="_blank">[${message("manage.common.view")}]<\/a>
					<a href="javascript:;" class="deleteGift">[${message("manage.common.delete")}]<\/a>
				<\/td>
			<\/tr>';
		[/@compress]
		$giftTitle.show();
	...
	$.validator.addMethod("compare", 
		...
		"${message("manage.promotion.compare")}"
	);
	...
	$inputForm.validate({
		rules: {
			...
			priceExpression: {
				remote: {
					url: "check_price_expression.do",
					cache: false
				}
			},
			pointExpression: {
				remote: {
					url: "check_point_expression.do",
					cache: false
				}
			},
			order: "digits"
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.promotion.edit")}
	</div>
	<form id="inputForm" action="update.do" method="post">
		<input type="hidden" name="id" value="${promotion.id}" />
		<ul id="tab" class="tab">
			<li>
				<input type="button" value="${message("manage.promotion.base")}" />
			</li>
			<li>
		...
		<div class="tabContent">
			<table class="input">
				...
				<tr>
					...
					<td colspan="2">
						<input type="text" name="priceExpression" class="text" value="${promotion.priceExpression}" maxlength="255" title="${message("manage.promotion.priceExpressionTitle")}" />
					</td>
				</tr>
				<tr>
					<th>
					...
					<td colspan="2">
						<input type="text" name="pointExpression" class="text" value="${promotion.pointExpression}" maxlength="255" title="${message("manage.promotion.pointExpressionTitle")}" />
					</td>
				</tr>
				<tr>
					<th>
			...
			<table id="productTable" class="input">
				<tr>
					...
					<td colspan="2">
						<input type="text" id="productSelect" name="productSelect" class="text" maxlength="200" title="${message("manage.promotion.productSelectTitle")}" />
					</td>
				</tr>
			...
			<table id="giftTable" class="input">
				<tr>
					...
					<td colspan="3">
						<input type="text" id="giftSelect" name="giftSelect" class="text" maxlength="200" title="${message("manage.promotion.giftSelectTitle")}" />
					</td>
				</tr>
				...
				[#list promotion.giftItems as giftItem]
					<tr class="giftTr">
						...
						<td>
							<a href="${base}${giftItem.gift.path}" target="_blank">[${message("manage.common.view")}]</a>
							<a href="javascript:;" class="deleteGift">[${message("manage.common.delete")}]</a>
						</td>
					</tr>
				[/#list]
