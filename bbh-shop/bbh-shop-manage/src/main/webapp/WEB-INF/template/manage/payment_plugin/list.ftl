...
<title>${message("manage.paymentPlugin.list")}</title>
...
$().ready(function() {
	...
	$install.click(function() {
		...
		$.dialog({
			type: "warn",
			content: "${message("manage.paymentPlugin.installConfirm")}",
			onOk: function() {
				$.ajax({
					url: $this.attr("href"),
					type: "POST",
	...
	$uninstall.click(function() {
		...
		$.dialog({
			type: "warn",
			content: "${message("manage.paymentPlugin.uninstallConfirm")}",
			onOk: function() {
				$.ajax({
					url: $this.attr("href"),
					type: "POST",
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.paymentPlugin.list")} <span>(${message("manage.page.total", paymentPlugins?size)})</span>
	</div>
					...
					<td>
						[#if paymentPlugin.isInstalled]
							[#if paymentPlugin.settingUrl??]
								<a href="${paymentPlugin.settingUrl}">[${message("manage.paymentPlugin.setting")}]</a>
							[/#if]
							[#if paymentPlugin.uninstallUrl??]
								<a href="${paymentPlugin.uninstallUrl}" class="uninstall">[${message("manage.paymentPlugin.uninstall")}]</a>
							[/#if]
						[#else]
							[#if paymentPlugin.installUrl??]
								<a href="${paymentPlugin.installUrl}" class="install">[${message("manage.paymentPlugin.install")}]</a>
							[/#if]
						[/#if]
					</td>
				</tr>
