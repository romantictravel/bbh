...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.do">${message("admin.path.index")}</a> &raquo; ${message("admin.area.list")}
	</div>
	<div class="bar">
		...
		[#list areas?chunk(5, "") as row]
			<tr>
				[#list row as area]
					[#if area?has_content]
						<td>
							<a href="list.do?parentId=${area.id}" title="${message("admin.common.view")}">${area.name}</a>
							<a href="edit.do?id=${area.id}">[${message("admin.common.edit")}]</a>
		...
		[#if !areas?has_content]
			<tr>
				<td colspan="5" style="text-align: center; color: red;">
					${message("admin.area.emptyChildren")} <a href="add.do[#if parent??]?parentId=${parent.id}[/#if]" style="color: gray">${message("admin.common.add")}</a>
				</td>
			</tr>
		[/#if]
	</table>
