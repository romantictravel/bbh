...
<title>${message("manage.area.add")}</title>
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.area.add")}
	</div>
	<form id="inputForm" action="save.do" method="post">
		...
		<table class="input">
			<tr>
				<th>
					${message("manage.area.parent")}:
				</th>
				<td>
					[#if parent??]${parent.name}[#else]${message("manage.area.root")}[/#if]
				</td>
			</tr>
			<tr>
				<th>
