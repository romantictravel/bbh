...
<title>${message("manage.friendLink.edit")}</title>
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.friendLink.edit")}
	</div>
			...
			<tr>
				...
				<td>
					<span class="fieldSet">
						...
						<input type="button" id="browserButton" class="button" value="${message("manage.browser.select")}"[#if friendLink.type == "text"] disabled="disabled"[/#if] />
