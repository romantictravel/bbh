...
var messages = {
	"manage.message.success": "${message("manage.message.success")}",
	"manage.message.error": "${message("manage.message.error")}",
	"manage.dialog.ok": "${message("manage.dialog.ok")}",
	"manage.dialog.cancel": "${message("manage.dialog.cancel")}",
	"manage.dialog.deleteConfirm": "${message("manage.dialog.deleteConfirm")}",
	"manage.dialog.clearConfirm": "${message("manage.dialog.clearConfirm")}",
	"manage.browser.title": "${message("manage.browser.title")}",
	"manage.browser.upload": "${message("manage.browser.upload")}",
	"manage.browser.parent": "${message("manage.browser.parent")}",
	"manage.browser.orderType": "${message("manage.browser.orderType")}",
	"manage.browser.name": "${message("manage.browser.name")}",
	"manage.browser.size": "${message("manage.browser.size")}",
	"manage.browser.type": "${message("manage.browser.type")}",
	"manage.browser.select": "${message("manage.browser.select")}",
	"manage.upload.sizeInvalid": "${message("manage.upload.sizeInvalid")}",
	"manage.upload.typeInvalid": "${message("manage.upload.typeInvalid")}",
	"manage.upload.invalid": "${message("manage.upload.invalid")}",
	"manage.validate.required": "${message("manage.validate.required")}",
	"manage.validate.email": "${message("manage.validate.email")}",
	"manage.validate.url": "${message("manage.validate.url")}",
	"manage.validate.date": "${message("manage.validate.date")}",
	"manage.validate.dateISO": "${message("manage.validate.dateISO")}",
	"manage.validate.pointcard": "${message("manage.validate.pointcard")}",
	"manage.validate.number": "${message("manage.validate.number")}",
	"manage.validate.digits": "${message("manage.validate.digits")}",
	"manage.validate.minlength": "${message("manage.validate.minlength")}",
	"manage.validate.maxlength": "${message("manage.validate.maxlength")}",
	"manage.validate.rangelength": "${message("manage.validate.rangelength")}",
	"manage.validate.min": "${message("manage.validate.min")}",
	"manage.validate.max": "${message("manage.validate.max")}",
	"manage.validate.range": "${message("manage.validate.range")}",
	"manage.validate.accept": "${message("manage.validate.accept")}",
	"manage.validate.equalTo": "${message("manage.validate.equalTo")}",
	"manage.validate.remote": "${message("manage.validate.remote")}",
	"manage.validate.integer": "${message("manage.validate.integer")}",
	"manage.validate.positive": "${message("manage.validate.positive")}",
	"manage.validate.negative": "${message("manage.validate.negative")}",
	"manage.validate.decimal": "${message("manage.validate.decimal")}",
	"manage.validate.pattern": "${message("manage.validate.pattern")}",
	"manage.validate.extension": "${message("manage.validate.extension")}"
};
...
(function($) {
	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				type: "image",
				title: message("manage.browser.title"),
				isUpload: true,
				browserUrl: kcshop.base + "/manage/file/browser.do",
				uploadUrl: kcshop.base + "/manage/file/upload.do",
				callback: null
			};
			$.extend(settings, options);
			...
			return this.each(function() {
				...
				$browserButton.click(function() {
					...
					if (settings.isUpload) {
						...
						$browserUploadButton = $('<a href="javascript:;" class="browserUploadButton button">' + message("manage.browser.upload") + '<\/a>').appendTo($browserForm);
						$browserUploadInput = $('<input type="file" name="file" \/>').appendTo($browserUploadButton);
					}
					$browserParentButton = $('<a href="javascript:;" class="button">' + message("manage.browser.parent") + '<\/a>').appendTo($browserBar);
					$browserBar.append(message("manage.browser.orderType") + ": ");
					$browserOrderType = $('<select name="orderType" class="browserOrderType"><option value="name">' + message("manage.browser.name") + '<\/option><option value="size">' + message("manage.browser.size") + '<\/option><option value="type">' + message("manage.browser.type") + '<\/option><\/select>').appendTo($browserBar);
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = kcshop.base + "/resources/manage/images/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = kcshop.base + "/resources/manage/images/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
					...
					$browserUploadInput.change(function() {
						...
						if ($.trim(allowedUploadExtensions) == "" || !new RegExp("^\\S.*\\.(" + allowedUploadExtensions.replace(/,/g, "|") + ")$", "i").test($browserUploadInput.val())) {
							$.message("warn", message("manage.upload.typeInvalid"));
							return false;
						}
						$browserLoadingIcon.show();
						$browserForm.submit();
	...
	$(document).ajaxComplete(function(event, request, settings) {
		...
		if (loginStatus == "accessDenied") {
			$.message("warn", "${message("manage.login.accessDenied")}");
			setTimeout(function() {
				location.reload(true);
			}, 2000);
		} else if (loginStatus == "unauthorized") {
			$.message("warn", "${message("manage.unauthorized.message")}");
