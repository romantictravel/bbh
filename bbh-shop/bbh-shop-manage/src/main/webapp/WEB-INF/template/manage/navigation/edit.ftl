...
<title>${message("manage.navigation.edit")}</title>
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.navigation.edit")}
	</div>
	<form id="inputForm" action="update.do" method="post">
		...
		<table class="input">
			...
			<tr>
				<th>
					${message("manage.navigation.systemUrl")}:
				</th>
				<td>
					<select id="systemUrl">
						<option value="">------------</option>
						<option value="${base}/"[#if navigation.url == base + "/"] selected="selected"[/#if]>${message("manage.navigation.home")}</option>
						<option value="${base}/product_category.do"[#if navigation.url == base + "/product_category.do"] selected="selected"[/#if]>${message("manage.navigation.productCategory")}</option>
						<option value="${base}/friend_link.do"[#if navigation.url == base + "/friend_link.do"] selected="selected"[/#if]>${message("manage.navigation.friendLink")}</option>
						<option value="${base}/member/index.do"[#if navigation.url == base + "/member/index.do"] selected="selected"[/#if]>${message("manage.navigation.member")}</option>
