...
<title>${message("manage.salesRanking.list")}</title>
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.salesRanking.list")}
	</div>
	<form id="listForm" action="list.do" method="get">
		...
		<div class="bar">
			<div class="buttonWrap">
				...
				<div class="menuWrap">
					<a href="javascript:;" id="countSelect" class="button">
						${message("manage.salesRanking.count")}<span class="arrow">&nbsp;</span>
					</a>
					<div class="popupMenu">
						<ul id="countOption">
							<li>
			...
			${message("manage.salesRanking.beginDate")}: <input type="text" id="beginDate" name="beginDate" class="text Wdate" value="${(beginDate?string('yyyy-MM-dd'))!}" onfocus="WdatePicker({maxDate: '#F{$dp.$D(\'endDate\')}'});" />
			${message("manage.salesRanking.endDate")}: <input type="text" id="endDate" name="endDate" class="text Wdate" value="${(endDate?string('yyyy-MM-dd'))!}" onfocus="WdatePicker({minDate: '#F{$dp.$D(\'beginDate\')}'});" />
			<input type="submit" class="button" value="${message("manage.common.submit")}" />
		</div>
		<table id="listTable" class="list">
			<tr>
				<th>
					<span>${message("manage.salesRanking.rank")}</span>
				</th>
				<th>
					<span>${message("manage.salesRanking.productSn")}</span>
				</th>
				<th>
					<span>${message("manage.salesRanking.productName")}</span>
				</th>
				<th>
					<span>${message("manage.salesRanking.salesVolume")}</span>
				</th>
				<th>
					<span>${message("manage.salesRanking.salesAmount")}</span>
				</th>
			</tr>
			[#list data as row]
				<tr>
