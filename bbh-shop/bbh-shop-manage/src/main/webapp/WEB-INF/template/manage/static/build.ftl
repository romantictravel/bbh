...
<title>${message("manage.static.build")}</title>
			...
			success: function(data) {
				...
				if (!data.isCompleted) {
					if (buildType == "article" || buildType == "product") {
						first = data.first;
						$status.text("${message("manage.static.building")} [" + first + " - " + (first + count) + "]");
					} else {
						$status.text("${message("manage.static.building")}");
					}
					build();
				} else {
					...
					var time;
					if (buildTime < 60000) {
						time = (buildTime / 1000).toFixed(2) + "${message("manage.static.second")}";
					} else {
						time = (buildTime / 60000).toFixed(2) + "${message("manage.static.minute")}";
					}
					$.message("success", "${message("manage.static.success")} [${message("manage.static.buildCount")}: " + buildCount + " ${message("manage.static.buildTime")}: " + time + "]");
				}
			}
		});
	}
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.static.build")}
	</div>
	<form id="inputForm" action="build.do" method="post">
		<table class="input">
			<tr>
				<th>
					${message("manage.static.buildType")}:
				</th>
				<td>
					<select id="buildType" name="buildType">
						[#list buildTypes as buildType]
							<option value="${buildType}">${message("manage.static." + buildType)}</option>
						[/#list]
					</select>
				</td>
			</tr>
			<tr id="articleCategoryTr" class="hidden">
				...
				<td>
					<select id="articleCategoryId" name="articleCategoryId">
						<option value="">${message("manage.static.emptyOption")}</option>
			...
			<tr id="beginDateTr" class="hidden">
				<th>
					${message("manage.static.beginDate")}:
				</th>
				<td>
					<input type="text" id="beginDate" name="beginDate" class="text Wdate" value="${defaultBeginDate?string("yyyy-MM-dd")}" title="${message("manage.static.beginDateTitle")}" onfocus="WdatePicker({maxDate: '#F{$dp.$D(\'endDate\')}'});" />
				</td>
			</tr>
			<tr id="endDateTr" class="hidden">
				<th>
					${message("manage.static.endDate")}:
				</th>
				<td>
					<input type="text" id="endDate" name="endDate" class="text Wdate" value="${defaultEndDate?string("yyyy-MM-dd")}" title="${message("manage.static.endDateTitle")}" onfocus="WdatePicker({minDate: '#F{$dp.$D(\'beginDate\')}'});" />
				</td>
			</tr>
			<tr>
				<th>
					<span class="requiredField">*</span>${message("manage.static.count")}:
				</th>
				<td>
