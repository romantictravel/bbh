...
<shiro:authenticated>
<%
response.sendRedirect(base + "/admin/common/main.do");
%>
</shiro:authenticated>
<%
}
...
%>
<title><%=SpringContextUtils.getMessage("admin.login.title")%></title>
...
<script type="text/javascript">
	$().ready( function() {
		...
		$loginForm.submit( function() {
			if ($username.val() == "") {
				$.message("warn", "<%=SpringContextUtils.getMessage("admin.login.usernameRequired")%>");
				return false;
			}
			if ($password.val() == "") {
				$.message("warn", "<%=SpringContextUtils.getMessage("admin.login.passwordRequired")%>");
				return false;
			}
			if ($captcha.val() == "") {
				$.message("warn", "<%=SpringContextUtils.getMessage("admin.login.captchaRequired")%>");
				return false;
			}
				...
				<table>
					<tr>
						<td width="190" rowspan="2" align="center" valign="bottom">
							<img src="<%=base%>/resources/admin/images/login_logo.gif" alt="KCSHOP" />
						</td>
						<th>
							<%=SpringContextUtils.getMessage("admin.login.username")%>:
						</th>
						<td>
					...
					<tr>
						<th>
							<%=SpringContextUtils.getMessage("admin.login.password")%>:
						</th>
						<td>
					...
					<%if (ArrayUtils.contains(setting.getCaptchaTypes(), CaptchaType.adminLogin)) {%>
						<tr>
							...
							<th>
								<%=SpringContextUtils.getMessage("admin.captcha.name")%>:
							</th>
							<td>
								<input type="text" id="captcha" name="captcha" class="text captcha" maxlength="4" autocomplete="off" /><img id="captchaImage" class="captchaImage" src="<%=base%>/admin/common/captcha.do?captchaId=<%=captchaId%>" title="<%=SpringContextUtils.getMessage("admin.captcha.imageTitle")%>" />
							</td>
						</tr>
					<%}%>
					<tr>
						...
						<td>
							<label>
								...
								<%=SpringContextUtils.getMessage("admin.login.rememberUsername")%>:
							</label>
						</td>
					</tr>
					<tr>
						...
						<td>
							<input type="button" class="homeButton" value="" onclick="location.href='<%=base%>/'" /><input type="submit" class="loginButton" value="<%=SpringContextUtils.getMessage("admin.login.login")%>" />
						</td>
					</tr>
				</table>
				<div class="powered">COPYRIGHT © 2005-2013 KCSHOP.NET ALL RIGHTS RESERVED.</div>
				<div class="link">
					<a href="<%=base%>/"><%=SpringContextUtils.getMessage("admin.login.home")%></a> |
					<a href="http://www.kcshop.net"><%=SpringContextUtils.getMessage("admin.login.official")%></a> |
					<a href="http://bbs.kcshop.net"><%=SpringContextUtils.getMessage("admin.login.bbs")%></a> |
					<a href="http://www.kcshop.net/about.html"><%=SpringContextUtils.getMessage("admin.login.about")%></a> |
					<a href="http://www.kcshop.net/contact.html"><%=SpringContextUtils.getMessage("admin.login.contact")%></a> |
					<a href="http://www.kcshop.net/license.html"><%=SpringContextUtils.getMessage("admin.login.license")%></a>
				</div>
			</form>
		</div>
	<%} else {%>
