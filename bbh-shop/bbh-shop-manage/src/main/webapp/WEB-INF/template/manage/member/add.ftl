...
<title>${message("manage.member.add")}</title>
	...
	$inputForm.validate({
		...
		messages: {
			username: {
				...
				remote: "${message("manage.member.disabledExist")}"
			},
			password: {
				pattern: "${message("manage.validate.illegal")}"
			}
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.member.add")}
	</div>
	<form id="inputForm" action="save.do" method="post">
		...
		<table class="input tabContent">
			...
			<tr>
				<th>
					<span class="requiredField">*</span>${message("manage.member.rePassword")}:
				</th>
				<td>
