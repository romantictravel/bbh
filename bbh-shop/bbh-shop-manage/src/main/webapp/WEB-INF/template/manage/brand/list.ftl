...
<title>${message("manage.brand.list")}</title>
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.brand.list")} <span>(${message("manage.page.total", page.total)})</span>
	</div>
		...
		<table id="listTable" class="list">
			...
			[#list page.content as brand]
				<tr>
					...
					<td>
						[#if brand.type == "image"]
							<a href="${brand.logo}" target="_blank">${message("manage.common.view")}</a>
						[#else]
							-
						[/#if]
					</td>
					...
					<td>
						<a href="edit.do?id=${brand.id}">[${message("manage.common.edit")}]</a>
						<a href="${base}${brand.path}" target="_blank">[${message("manage.common.view")}]</a>
					</td>
				</tr>
			[/#list]
		</table>
