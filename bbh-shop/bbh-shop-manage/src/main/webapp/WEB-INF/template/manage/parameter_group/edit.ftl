...
<title>${message("manage.parameterGroup.edit")}</title>
...
$().ready(function() {
	...
	$addParameter.live("click", function() {
		...
		[@compress single_line = true]
			...
			'<tr class="parameterTr">
				...
				<\/td>
				<td>
					<a href="javascript:;" class="deleteParameter">[${message("manage.common.delete")}]<\/a>
				<\/td>
			<\/tr>';
		[/@compress]
	...
	$deleteParameter.live("click", function() {
		...
		if ($parameterTable.find("tr.parameterTr").size() <= 1) {
			$.message("warn", "${message("manage.parameterGroup.deleteAllParameterNotAllowed")}");
		} else {
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.parameterGroup.edit")}
	</div>
	<form id="inputForm" action="update.do" method="post">
		...
		<table id="parameterTable" class="input">
			...
			<tr>
				...
				<td colspan="3">
					<a href="javascript:;" id="addParameter" class="button">${message("manage.parameterGroup.addParameter")}</a>
				</td>
			</tr>
			<tr class="title">
				<td>
