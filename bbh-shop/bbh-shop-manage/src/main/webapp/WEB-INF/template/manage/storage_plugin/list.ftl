...
<title>${message("manage.storagePlugin.list")}</title>
...
$().ready(function() {
	...
	$install.click(function() {
		...
		$.dialog({
			type: "warn",
			content: "${message("manage.storagePlugin.installConfirm")}",
			onOk: function() {
				$.ajax({
					url: $this.attr("href"),
					type: "POST",
	...
	$uninstall.click(function() {
		...
		$.dialog({
			type: "warn",
			content: "${message("manage.storagePlugin.uninstallConfirm")}",
			onOk: function() {
				$.ajax({
					url: $this.attr("href"),
					type: "POST",
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.storagePlugin.list")} <span>(${message("manage.page.total", storagePlugins?size)})</span>
	</div>
					...
					<td>
						[#if storagePlugin.isInstalled]
							[#if storagePlugin.settingUrl??]
								<a href="${storagePlugin.settingUrl}">[${message("manage.storagePlugin.setting")}]</a>
							[/#if]
							[#if storagePlugin.uninstallUrl??]
								<a href="${storagePlugin.uninstallUrl}" class="uninstall">[${message("manage.storagePlugin.uninstall")}]</a>
							[/#if]
						[#else]
							[#if storagePlugin.installUrl??]
								<a href="${storagePlugin.installUrl}" class="install">[${message("manage.storagePlugin.install")}]</a>
							[/#if]
						[/#if]
					</td>
				</tr>
