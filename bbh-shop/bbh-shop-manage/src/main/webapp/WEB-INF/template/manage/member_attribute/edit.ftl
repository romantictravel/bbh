...
<title>${message("manage.memberAttribute.edit")}</title>
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.memberAttribute.edit")}
	</div>
		...
		<table id="memberAttributeTable" class="input">
			...
			[#if memberAttribute.type?? && (memberAttribute.type == "select" || memberAttribute.type == "checkbox")]
				<tr>
					...
					<td>
						<a href="javascript:;" id="addOptionButton" class="button">${message("manage.common.add")}</a>
					</td>
				</tr>
			[/#if]
