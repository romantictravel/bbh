...
<title>${message("manage.order.edit")}</title>
...
$().ready(function() {
	...
	// 地区选择
	$areaId.lSelect({
		url: "${base}/manage/common/area.do"
	});
	...
	$addOrderItem.click(function() {
		...
		if (productSn == "") {
			$.message("warn", "${message("manage.order.productSnRequired")}");
			return false;
		}
		var repeat = false;
		...
		if (repeat) {
			$.message("warn", "${message("manage.order.productSnExist")}");
			return false;
		}
		
		$.ajax({
			url: "order_item_add.do",
			type: "POST",
					...
					[@compress single_line = true]
						...
						'<tr class="orderItemTr">
							...
							<td width="400">
								...
								' + (data.isGift ? '<span class="red">[${message("manage.order.gift")}]<\/span>' : '') + '
							<\/td>
							<td>
							...
							<\/td>
							<td>
								<a href="javascript:;" class="deleteOrderItem">[${message("manage.common.delete")}]<\/a>
							<\/td>
						<\/tr>');
					[/@compress]
	...
	$deleteOrderItem.live("click", function() {
		...
		if ($orderItemTable.find("tr.orderItemTr").size() <= 1) {
			$.message("warn", "${message("manage.common.deleteAllNotAllowed")}");
		} else {
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.order.edit")}
	</div>
	<ul id="tab" class="tab">
		<li>
	...
	<form id="inputForm" action="update.do" method="post">
		...
		<table class="input tabContent">
			...
			<tr>
				...
				<td>
					<a href="../member/view.do?id=${order.member.id}">${order.member.username}</a>
				</td>
			</tr>
			<tr>
				<th>
		...
		<table id="orderItemTable" class="input tabContent">
			<tr>
				<td colspan="6">
					${message("manage.order.productSn")}: <input type="text" id="productSn" name="productSn" class="text" maxlength="200" />
					<input type="button" id="addOrderItem" class="button" value="${message("manage.order.addProduct")}" />
				</td>
			</tr>
			<tr class="title">
				<th>
				...
				<th>
					${message("manage.order.productPrice")}
				</th>
				<th>
					${message("manage.order.productQuantity")}
				</th>
				<th>
					${message("manage.order.subTotal")}
				</th>
				<th>
