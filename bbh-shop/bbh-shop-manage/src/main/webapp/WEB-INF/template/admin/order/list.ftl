...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.do">${message("admin.path.index")}</a> &raquo; ${message("admin.order.list")} <span>(${message("admin.page.total", page.total)})</span>
	</div>
					...
					[@shiro.hasPermission name = "admin:print"]
						<td>
							<select name="print">
								...
								<option value="../print/order.do?id=${order.id}">${message("admin.order.orderPrint")}</option>
								<option value="../print/product.do?id=${order.id}">${message("admin.order.productPrint")}</option>
								<option value="../print/shipping.do?id=${order.id}">${message("admin.order.shippingPrint")}</option>
								<option value="../print/delivery.do?orderId=${order.id}">${message("admin.order.deliveryPrint")}</option>
							</select>
						</td>
					[/@shiro.hasPermission]
					<td>
						<a href="view.do?id=${order.id}">[${message("admin.common.view")}]</a>
						[#if !order.expired && order.orderStatus == "unconfirmed"]
							<a href="edit.do?id=${order.id}">[${message("admin.common.edit")}]</a>
						[#else]
