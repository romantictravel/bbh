...
<title>${message("manage.statistics.setting")}</title>
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.statistics.setting")}
	</div>
	<form action="setting.do" method="post">
		<table class="input">
			<tr>
				<th>
					${message("manage.statistics.isEnabled")}:
				</th>
				<td>
