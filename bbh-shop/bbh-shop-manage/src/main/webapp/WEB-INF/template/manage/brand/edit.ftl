...
<title>${message("manage.brand.edit")}</title>
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.brand.edit")}
	</div>
			...
			<tr>
				...
				<td>
					<span class="fieldSet">
						...
						<input type="button" id="browserButton" class="button" value="${message("manage.browser.select")}"[#if brand.type == "text"] disabled="disabled"[/#if] />
						[#if brand.type == "image"]
