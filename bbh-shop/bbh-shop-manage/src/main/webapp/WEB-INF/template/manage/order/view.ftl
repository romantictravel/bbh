...
<title>${message("manage.order.view")}</title>
...
<script type="text/javascript" src="${base}/resources/manage/js/jquery.lSelect.js"></script>
...
$().ready(function() {
	...
	$confirmButton.click(function() {
		...
		$.dialog({
			type: "warn",
			content: "${message("manage.order.confirmDialog")}",
			onOk: function() {
				$confirmForm.submit();
			}
		});
	...
	$completeButton.click(function() {
		...
		$.dialog({
			type: "warn",
			content: "${message("manage.order.completeDialog")}",
			onOk: function() {
				$completeForm.submit();
			}
		});
	...
	$cancelButton.click(function() {
		...
		$.dialog({
			type: "warn",
			content: "${message("manage.order.cancelDialog")}",
			onOk: function() {
				$cancelForm.submit();
			}
		});
	...
	[#if !order.expired && order.orderStatus == "confirmed" && (order.paymentStatus == "unpaid" || order.paymentStatus == "partialPayment")]
		...
		$paymentButton.click(function() {
			$.dialog({
				title: "${message("manage.order.payment")}",
				...
				onShow: function() {
					...
					$.validator.addMethod("balance", 
						...
						"${message("manage.order.insufficientBalance")}"
					);
	...
	[#if !order.expired && order.orderStatus == "confirmed" && (order.paymentStatus == "paid" || order.paymentStatus == "partialPayment" || order.paymentStatus == "partialRefunds")]
		...
		$refundsButton.click(function() {
			$.dialog({
				title: "${message("manage.order.refunds")}",
				[@compress single_line = true]
					content: '
					<form id="refundsForm" action="refunds.do" method="post">
						<input type="hidden" name="token" value="${token}" \/>
						<input type="hidden" name="orderId" value="${order.id}" \/>
						<table class="input">
							<tr>
	...
	[#if !order.expired && order.orderStatus == "confirmed" && (order.shippingStatus == "unshipped" || order.shippingStatus == "partialShipment")]
		...
		$shippingButton.click(function() {
			$.dialog({
				title: "${message("manage.order.shipping")}",
				[@compress single_line = true]
					content: '
					<form id="shippingForm" action="shipping.do" method="post">
						<input type="hidden" name="token" value="${token}" \/>
						<input type="hidden" name="orderId" value="${order.id}" \/>
						<div style="height: 380px; overflow-x: hidden; overflow-y: auto;">
							...
							<table class="input">
								<tr class="title">
									...
									<\/th>
									<th>
										${message("manage.order.productStock")}
									<\/th>
									<th>
										${message("manage.order.productQuantity")}
									<\/th>
									<th>
										${message("OrderItem.shippedQuantity")}
									<\/th>
									<th>
										${message("manage.order.shippingQuantity")}
									<\/th>
								<\/tr>
								[#list order.orderItems as orderItem]
									<tr>
										...
										<td width="300">
											...
											[#if orderItem.isGift]
												<span class="red">[${message("manage.order.gift")}]<\/span>
											[/#if]
										<\/td>
										<td>
				...
				onOk: function() {
					...
					if (total <= 0) {
						$.message("warn", "${message("manage.order.shippingQuantityPositive")}");
					} else {
						$("#shippingForm").submit();
					}
					return false;
	...
	[#if !order.expired && order.orderStatus == "confirmed" && (order.shippingStatus == "shipped" || order.shippingStatus == "partialShipment" || order.shippingStatus == "partialReturns")]
		...
		$returnsButton.click(function() {
			$.dialog({
				title: "${message("manage.order.returns")}",
				[@compress single_line = true]
					content: '
					<form id="returnsForm" action="returns.do" method="post">
						<input type="hidden" name="token" value="${token}" \/>
						<input type="hidden" name="orderId" value="${order.id}" \/>
						<div style="height: 380px; overflow-x: hidden; overflow-y: auto;">
							<table class="input" style="margin-bottom: 30px;">
							...
							<table class="input">
								<tr class="title">
									...
									<\/th>
									<th>
										${message("manage.order.returnsQuantity")}
									<\/th>
								<\/tr>
								[#list order.orderItems as orderItem]
									<tr>
				...
				onOk: function() {
					...
					if (total <= 0) {
						$.message("warn", "${message("manage.order.returnsQuantityPositive")}");
					} else {
						$("#returnsForm").submit();
					}
					return false;
...
<body>
	...
	<form id="completeForm" action="complete.do" method="post">
		<input type="hidden" name="id" value="${order.id}" />
	</form>
	<form id="cancelForm" action="cancel.do" method="post">
		<input type="hidden" name="id" value="${order.id}" />
	</form>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.order.view")}
	</div>
	<ul id="tab" class="tab">
		<li>
			<input type="button" value="${message("manage.order.orderInfo")}" />
		</li>
		<li>
			<input type="button" value="${message("manage.order.productInfo")}" />
		</li>
		<li>
			<input type="button" value="${message("manage.order.paymentInfo")}" />
		</li>
		<li>
			<input type="button" value="${message("manage.order.shippingInfo")}" />
		</li>
		<li>
			<input type="button" value="${message("manage.order.refundsInfo")}" />
		</li>
		<li>
			<input type="button" value="${message("manage.order.returnsInfo")}" />
		</li>
		<li>
			<input type="button" value="${message("manage.order.orderLog")}" />
		</li>
	</ul>
	<table class="input tabContent">
		<tr>
			...
			<td>
				<input type="button" id="confirmButton" class="button" value="${message("manage.order.confirm")}"[#if order.expired || order.orderStatus != "unconfirmed"] disabled="disabled"[/#if] />
				<input type="button" id="paymentButton" class="button" value="${message("manage.order.payment")}"[#if order.expired || order.orderStatus != "confirmed" || (order.paymentStatus != "unpaid" && order.paymentStatus != "partialPayment")] disabled="disabled"[/#if] />
				<input type="button" id="shippingButton" class="button" value="${message("manage.order.shipping")}"[#if order.expired || order.orderStatus != "confirmed" || (order.shippingStatus != "unshipped" && order.shippingStatus != "partialShipment")] disabled="disabled"[/#if] />
				<input type="button" id="completeButton" class="button" value="${message("manage.order.complete")}"[#if order.expired || order.orderStatus != "confirmed"] disabled="disabled"[/#if] />
			</td>
			<td>
				&nbsp;
			</td>
			<td>
				<input type="button" id="refundsButton" class="button" value="${message("manage.order.refunds")}"[#if order.expired || order.orderStatus != "confirmed" || (order.paymentStatus != "paid" && order.paymentStatus != "partialPayment" && order.paymentStatus != "partialRefunds")] disabled="disabled"[/#if] />
				<input type="button" id="returnsButton" class="button" value="${message("manage.order.returns")}"[#if order.expired || order.orderStatus != "confirmed" || (order.shippingStatus != "shipped" && order.shippingStatus != "partialShipment" && order.shippingStatus != "partialReturns")] disabled="disabled"[/#if] />
				<input type="button" id="cancelButton" class="button" value="${message("manage.order.cancel")}"[#if order.expired || order.orderStatus != "unconfirmed"] disabled="disabled"[/#if] />
			</td>
		</tr>
		<tr>
			<th>
		...
		<tr>
			...
			<td>
				...
				[#if order.expired]
					<span title="${message("Order.expire")}: ${order.expire?string("yyyy-MM-dd HH:mm:ss")}">(${message("manage.order.hasExpired")})</span>
				[#elseif order.expire??]
			...
			<tr>
				...
				<td>
					[#if orderLog.operator??]
						${orderLog.operator}
					[#else]
						<span class="green">${message("manage.order.member")}</span>
					[/#if]
				</td>
				<td>
					[#if orderLog.content??]
