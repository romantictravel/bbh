...
<title>${message("manage.member.edit")}</title>
		...
		rules: {
			...
			email: {
				...
				[#if !setting.isDuplicateEmail]
					,remote: {
						url: "check_email.do?previousEmail=${member.email?url}",
						cache: false
					}
				[/#if]
			},
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.member.edit")}
	</div>
	<form id="inputForm" action="update.do" method="post">
		<input type="hidden" name="id" value="${member.id}" />
		<ul id="tab" class="tab">
			<li>
				<input type="button" value="${message("manage.member.base")}" />
			</li>
			[#if memberAttributes?has_content]
				<li>
					<input type="button" value="${message("manage.member.profile")}" />
				</li>
			[/#if]
			<li>
				<input type="button" value="${message("manage.member.point")}" />
			</li>
			<li>
				<input type="button" value="${message("manage.member.deposit")}" />
			</li>
		</ul>
		<table class="input tabContent">
			...
			<tr>
				...
				<td>
					<input type="password" id="password" name="password" class="text" maxlength="20" title="${message("manage.member.passwordTitle")}" />
				</td>
			</tr>
			<tr>
				<th>
					${message("manage.member.rePassword")}:
				</th>
				<td>
		...
		<table class="input tabContent">
			<tr>
				<th>
					${message("manage.member.currentPoint")}:
				</th>
				<td>
					${member.point}
				</td>
			...
			<tr>
				<th>
					${message("manage.member.modifyPoint")}:
				</th>
				<td>
					<input type="text" name="modifyPoint" class="text" maxlength="9" title="${message("manage.member.modifyPointTitle")}" />
				</td>
			</tr>
		</table>
		<table class="input tabContent">
			<tr>
				<th>
					${message("manage.member.currentBalance")}:
				</th>
				<td>
			...
			<tr>
				<th>
					${message("manage.member.modifyBalance")}:
				</th>
				<td>
					<input type="text" name="modifyBalance" class="text" maxlength="16" title="${message("manage.member.modifyBalanceTitle")}" />
				</td>
			</tr>
			<tr>
				<th>
					${message("manage.member.depositMemo")}:
				</th>
				<td>
