...
<title>${message("manage.index.build")}</title>
	...
	function build() {
		$.ajax({
			...
			success: function(data) {
				...
				if (!data.isCompleted) {
					first = data.first;
					$status.text("${message("manage.index.building")} [" + first + " - " + (first + count) + "]");
					build();
				} else {
					...
					var time;
					if (buildTime < 60000) {
						time = (buildTime / 1000).toFixed(2) + "${message("manage.index.second")}";
					} else {
						time = (buildTime / 60000).toFixed(2) + "${message("manage.index.minute")}";
					}
					$.message("success", "${message("manage.index.success")} [${message("manage.index.buildCount")}: " + buildCount + " ${message("manage.index.buildTime")}: " + time + "]");
				}
			}
		});
	}
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.index.build")}
	</div>
	<form id="inputForm" action="build.do" method="post">
		<table class="input">
			<tr>
				<th>
					${message("manage.index.buildType")}:
				</th>
				<td>
					<select id="buildType" name="buildType">
						[#list buildTypes as buildType]
							<option value="${buildType}">${message("manage.index." + buildType)}</option>
						[/#list]
					</select>
				</td>
			</tr>
			<tr>
				<th>
					<span class="requiredField">*</span>${message("manage.index.count")}:
				</th>
				<td>
