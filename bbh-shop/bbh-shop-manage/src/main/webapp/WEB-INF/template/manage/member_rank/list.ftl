...
<title>${message("manage.memberRank.list")}</title>
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.memberRank.list")} <span>(${message("manage.page.total", page.total)})</span>
	</div>
	<form id="listForm" action="list.do" method="get">
		...
		<table id="listTable" class="list">
			...
			[#list page.content as memberRank]
				<tr>
					<td>
						<input type="checkbox" name="ids"[#if memberRank.isDefault] title="${message("manage.memberRank.deleteDefaultNotAllowed")}" disabled="disabled"[#else] value="${memberRank.id}"[/#if] />
					</td>
					<td>
						${memberRank.name}
					</td>
					...
					<td>
						${message(memberRank.isDefault?string('manage.common.true', 'manage.common.false'))}
					</td>
					<td>
						<a href="edit.do?id=${memberRank.id}">[${message("manage.common.edit")}]</a>
					</td>
				</tr>
			[/#list]
		</table>
