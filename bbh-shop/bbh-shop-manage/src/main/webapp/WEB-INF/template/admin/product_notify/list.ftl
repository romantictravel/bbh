...
<title>${message("admin.productNotify.list")}</title>
...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.do">${message("admin.path.index")}</a> &raquo; ${message("admin.productNotify.list")} <span>(${message("admin.page.total", page.total)})</span>
	</div>
				...
				<tr>
					...
					<td>
						[#if productNotify.member??]
							<a href="${base}/admin/member/view.do?id=${productNotify.member.id}">${productNotify.member.username}</a>
						[#else]
							-
						[/#if]
					</td>
