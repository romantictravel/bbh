...
<title>${message("manage.deposit.list")}</title>
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.deposit.list")} <span>(${message("manage.page.total", page.total)})</span>
	</div>
	<form id="listForm" action="list.do" method="get">
		...
		<div class="bar">
			<div class="buttonWrap">
				[#if member??]
					<a href="../member/view.do?id=${member.id}" class="button">${message("manage.common.back")}</a>
				[/#if]
		...
		<table id="listTable" class="list">
			...
			[#list page.content as deposit]
				<tr>
					...
					<td>
						${(deposit.operator)!message("manage.deposit.member")}
					</td>
					<td>
						[#if deposit.member??]
							<a href="../member/view.do?id=${deposit.member.id}">${deposit.member.username}</a>
						[#else]
							-
						[/#if]
					</td>
					<td>
						[#if deposit.order??]
							<a href="../order/view.do?id=${deposit.order.id}">${deposit.order.sn}</a>
						[#else]
							-
						[/#if]
					</td>
					<td>
						[#if deposit.payment??]
							<a href="../payment/view.do?id=${deposit.payment.id}">${deposit.payment.sn}</a>
						[#else]
							-
						[/#if]
					</td>
