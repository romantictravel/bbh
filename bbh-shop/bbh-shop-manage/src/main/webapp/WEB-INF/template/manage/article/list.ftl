...
<title>${message("manage.article.list")}</title>
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.article.list")} <span>(${message("manage.page.total", page.total)})</span>
	</div>
		...
		<table id="listTable" class="list">
			...
			[#list page.content as article]
				<tr>
					...
					<td>
						<a href="edit.do?id=${article.id}">[${message("manage.common.edit")}]</a>
						[#if article.isPublication]
							<a href="${base}${article.path}" target="_blank">[${message("manage.common.view")}]</a>
						[#else]
							[${message("manage.article.notPublished")}]
						[/#if]
					</td>
				</tr>
			[/#list]
