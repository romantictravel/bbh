...
<title>${message("manage.order.list")}</title>
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.order.list")} <span>(${message("manage.page.total", page.total)})</span>
	</div>
	<form id="listForm" action="list.do" method="get">
			...
			<div class="buttonWrap">
				...
				<div class="menuWrap">
					<a href="javascript:;" id="filterSelect" class="button">
						${message("manage.order.filter")}<span class="arrow">&nbsp;</span>
					</a>
					<div class="popupMenu">
						<ul id="filterOption" class="check">
							...
							<li class="separator">
								<a href="javascript:;" name="hasExpired" val="true"[#if hasExpired?? && hasExpired] class="checked"[/#if]>${message("manage.order.hasExpired")}</a>
							</li>
							<li>
								<a href="javascript:;" name="hasExpired" val="false"[#if hasExpired?? && !hasExpired] class="checked"[/#if]>${message("manage.order.unexpired")}</a>
							</li>
						</ul>
					</div>
				</div>
		...
		<table id="listTable" class="list">
			<tr>
				...
				[@shiro.hasPermission name = "manage:print"]
					<th>
						<span>${message("manage.order.print")}</span>
					</th>
				[/@shiro.hasPermission]
				<th>
			...
			[#list page.content as order]
				<tr>
					...
					<td>
						...
						[#if order.expired]<span class="gray">(${message("manage.order.hasExpired")})</span>[/#if]
					</td>
					<td>
					...
					[@shiro.hasPermission name = "manage:print"]
						<td>
							<select name="print">
								...
								<option value="../print/order.do?id=${order.id}">${message("manage.order.orderPrint")}</option>
								<option value="../print/product.do?id=${order.id}">${message("manage.order.productPrint")}</option>
								<option value="../print/shipping.do?id=${order.id}">${message("manage.order.shippingPrint")}</option>
								<option value="../print/delivery.do?orderId=${order.id}">${message("manage.order.deliveryPrint")}</option>
							</select>
						</td>
					[/@shiro.hasPermission]
					<td>
						<a href="view.do?id=${order.id}">[${message("manage.common.view")}]</a>
						[#if !order.expired && order.orderStatus == "unconfirmed"]
							<a href="edit.do?id=${order.id}">[${message("manage.common.edit")}]</a>
						[#else]
							<span title="${message("manage.order.editNotAllowed")}">[${message("manage.common.edit")}]</span>
						[/#if]
					</td>
				</tr>
			[/#list]
