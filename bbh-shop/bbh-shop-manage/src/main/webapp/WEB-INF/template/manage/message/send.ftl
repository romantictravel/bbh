...
<title>${message("manage.message.send")}</title>
...
$().ready(function() {
	...
	$inputForm.validate({
		...
		messages: {
			username: {
				remote: "${message("manage.message.memberNotExsit")}"
			}
		}
	});
});
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.message.send")}
	</div>
	<form id="inputForm" action="send.do" method="post">
		...
		<table class="input">
			...
			<tr>
				...
				<td>
					<input type="button" id="send" class="button" value="${message("manage.message.submit")}" />
					[#if !draftMessage??]
						<input type="button" id="save" class="button" value="${message("manage.message.saveDraft")}" />
					[/#if]
				</td>
			</tr>
		</table>
