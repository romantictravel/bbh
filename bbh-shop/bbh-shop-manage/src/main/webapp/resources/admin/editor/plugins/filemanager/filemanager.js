...
KindEditor.plugin("fileadminr", function(K) {

	var self = this;
	var name = "fileadminr";
	var fileManagerJson = K.undef(self.fileManagerJson, self.basePath + "php/file_adminr_json.php");
	...
	self.plugin.fileadminrDialog = function(options) {
		...
		var html = [
			...
			'<div class="ke-plugin-fileadminr-header">',
			'<div class="ke-left">',
			...
			'</div>',
			'<div class="ke-plugin-fileadminr-body"></div>',
			'</div>'
		].join("");
		...
		div = dialog.div,
		bodyDiv = K(".ke-plugin-fileadminr-body", div),
