...
<title>${message("manage.product.edit")}</title>
...
$().ready(function() {
	...
	$addProductImage.click(function() {
		[@compress single_line = true]
			...
			'<tr>
				...
				<\/td>
				<td>
					<a href="javascript:;" class="deleteProductImage">[${message("manage.common.delete")}]<\/a>
				<\/td>
			<\/tr>';
		[/@compress]
	...
	$productCategoryId.change(function() {
		...
		if (hasValue) {
			$.dialog({
				type: "warn",
				content: "${message("manage.product.productCategoryChangeConfirm")}",
				width: 450,
				onOk: function() {
					loadParameter();
					loadAttribute();
		...
		$.ajax({
			...
			success: function(data) {
				...
				$.each(data, function(i, attribute) {
					var optionHtml = '<option value="">${message("manage.common.choose")}<\/option>';
	...
	$addSpecificationProduct.click(function() {
		if ($specificationIds.filter(":checked").size() == 0) {
			$.message("warn", "${message("manage.product.specificationRequired")}");
			return false;
		}
		if ($specificationProductTable.find("tr:gt(1)").size() == 0) {
			...
			$tr.find("td:first").text("${message("manage.product.currentSpecification")}");
	...
	$inputForm.validate({
		rules: {
			...
			sn: {
				pattern: /^[0-9a-zA-Z_-]+$/,
				remote: {
					url: "check_sn.do?previousSn=${product.sn}",
					cache: false
				}
			},
			price: {
		...
		submitHandler: function(form) {
			if ($specificationIds.filter(":checked").size() > 0 && $specificationProductTable.find("tr:gt(1)").size() == 0) {
				$.message("warn", "${message("manage.product.specificationProductRequired")}");
				return false;
			} else {
				var isRepeats = false;
				...
				$specificationProductTable.find("tr:gt(1)").each(function() {
					...
					if ($.inArray(parameter, parameters) >= 0) {
						$.message("warn", "${message("manage.product.specificationValueRepeat")}");
						isRepeats = true;
						return false;
					} else {
...
<body>
	<div class="path">
		<a href="${base}/manage/common/index.do">${message("manage.path.index")}</a> &raquo; ${message("manage.product.edit")}
	</div>
	<form id="inputForm" action="update.do" method="post" enctype="multipart/form-data">
		<input type="hidden" name="id" value="${product.id}" />
		<ul id="tab" class="tab">
			<li>
				<input type="button" value="${message("manage.product.base")}" />
			</li>
			<li>
				<input type="button" value="${message("manage.product.introduction")}" />
			</li>
			<li>
				<input type="button" value="${message("manage.product.productImage")}" />
			</li>
			<li>
				<input type="button" value="${message("manage.product.parameter")}" />
			</li>
			<li>
				<input type="button" value="${message("manage.product.attribute")}" />
			</li>
			<li>
				<input type="button" value="${message("manage.product.specification")}" />
			</li>
		</ul>
		<table class="input tabContent">
			...
			<tr>
				...
				<td>
					<input type="text" name="sn" class="text" value="${product.sn}" maxlength="100" title="${message("manage.product.snTitle")}" />
				</td>
			</tr>
			<tr>
				<th>
			...
			<tr>
				...
				<td>
					<label>
						<input type="checkbox" id="isMemberPrice" name="isMemberPrice" value="true"[#if product.memberPrice?has_content] checked="checked"[/#if] />${message("manage.product.isMemberPrice")}
					</label>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<input type="text" name="cost" class="text" value="${product.cost}" maxlength="16" title="${message("manage.product.costTitle")}" />
				</td>
			</tr>
			<tr>
				<th>
				...
				<td>
					<input type="text" name="marketPrice" class="text" value="${product.marketPrice}" maxlength="16" title="${message("manage.product.marketPriceTitle")}" />
				</td>
			</tr>
			<tr>
				<th>
				...
				<td>
					<span class="fieldSet">
						<input type="text" name="image" class="text" value="${product.image}" maxlength="200" title="${message("manage.product.imageTitle")}" />
						<input type="button" id="browserButton" class="button" value="${message("manage.browser.select")}" />
						[#if product.image??]
							<a href="${product.image}" target="_blank">${message("manage.common.view")}</a>
						[/#if]
					</span>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<input type="text" name="weight" class="text" value="${product.weight}" maxlength="9" title="${message("manage.product.weightTitle")}" />
				</td>
			</tr>
			<tr>
				<th>
				...
				<td>
					<input type="text" name="stock" class="text" value="${product.stock}" maxlength="9" title="${message("manage.product.stockTitle")}" />
				</td>
			</tr>
			<tr>
				<th>
			...
			<tr>
				...
				<td>
					<input type="text" name="point" class="text" value="${product.point}" maxlength="9" title="${message("manage.product.pointTitle")}" />
				</td>
			</tr>
			<tr>
				<th>
			...
			<tr>
				...
				<td>
					<input type="text" name="keyword" class="text" value="${product.keyword}" maxlength="200" title="${message("manage.product.keywordTitle")}" />
				</td>
			</tr>
			<tr>
				<th>
		...
		<table id="productImageTable" class="input tabContent">
			<tr>
				<td colspan="4">
					<a href="javascript:;" id="addProductImage" class="button">${message("manage.product.addProductImage")}</a>
				</td>
			</tr>
			<tr class="title">
				<th>
			...
			[#list product.productImages as productImage]
				<tr>
					<td>
						...
						<a href="${productImage.large}" target="_blank">${message("manage.common.view")}</a>
					</td>
					<td>
		...
		<table class="input tabContent">
			<tr class="title">
				<th>
					${message("manage.product.selectSpecification")}:
				</th>
			</tr>
			<tr>
				<td>
			...
			<tr>
				<td>
					<a href="javascript:;" id="addSpecificationProduct" class="button">${message("manage.product.addSpecificationProduct")}</a>
				</td>
			</tr>
			<tr>
				<td>
					<table id="specificationProductTable" class="input">
						...
						<tr class="hidden">
							...
							[/#list]
							<td>
								<a href="javascript:;" class="deleteSpecificationProduct">[${message("manage.common.delete")}]</a>
							</td>
						</tr>
						[#if product.specifications?has_content]
							<tr>
								<td>
									${message("manage.product.currentSpecification")}
						...
						[#list product.siblings as specificationProduct]
							<tr>
								...
								<td>
									<a href="javascript:;" class="deleteSpecificationProduct">[${message("manage.common.delete")}]</a>
									<a href="edit.do?id=${specificationProduct.id}">[${message("manage.common.edit")}]</a>
								</td>
							</tr>
						[/#list]
					</table>
